...
%         sigma = mean(abs(Y_wpe(:, FrameNo, reference_mic)).^2, 'all');
    ...
    % compute Rss %
    Y_bar_Y_bar_matrix = zeros(MicNum*(Lw_choose-b+2), MicNum*(Lw_choose-b+2), L);
    for FrameNo = start_ini_frame+L-1:ini_frame
        for ii = 1:L
            Y_bar_Y_bar_matrix(:, :, ii) = Y_bar_Y_bar_matrix(:, :, ii) + Y_bar(:, FrameNo)*Y_bar(:, FrameNo-ii+1)';
        end

    end

    Rss_first_row = zeros(1, L);
    for ii = 1:L
        Rss_first_row(:, ii) = w_bar'*Y_bar_Y_bar_matrix(:, :, ii)*w_bar;
    end

    Rss = zeros(L, L);
    Rss(1, :) = Rss_first_row;
    for ii = 2:L
        Rss(ii, :) = [flip(conj(Rss_first_row(:, 2:ii))) Rss_first_row(:, 1:L+1-ii)];
    end

    % compute Rsy %
    Y_bar_Y_matrix =  zeros(MicNum*(Lw_choose-b+2), MicNum, L);
    for FrameNo = start_ini_frame+L-1:ini_frame
        for ii = 1:L
            Y_bar_Y_matrix(:, :, ii) = Y_bar_Y_matrix(:, :, ii) + Y_bar(:, FrameNo-ii+1)*squeeze(Y_delay(n, FrameNo, :))';
        end
    ...
    Rsy = zeros(L, MicNum);
    for ii = 1:L
        Rsy(ii, :) = w_bar'*Y_bar_Y_matrix(:, :, ii);
    end
...
source_WPD_filemane_str = ['wav_WPD\source_WPD_all_Wiener-', string(reverberation_time), '.wav'];
...
source_WPD_filemane_str = ['wav_WPD\source_WPD_partial_Wiener-', string(reverberation_time), '.wav'];
source_WPD_filemane = join(source_WPD_filemane_str, '');
