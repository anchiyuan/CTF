...
Y_nodelay = zeros(frequency, NumOfFrame, MicNum);
for i = 1:MicNum
    Y_nodelay(:, :, i) = my_STFT(y_nodelay(i, :), NFFT, hopsize, win);
end
...
source_MINT_filemane_str = ['wav\source_predict_all_Wiener_MINT-', string(reverberation_time), 'x', string(g_len), 'x', string(weight_len),...
