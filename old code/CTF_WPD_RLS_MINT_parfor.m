...
error = zeros(MicNum, (ini_frame-(start_ini_frame+L-1)+1), frequency);
...
%         sigma = mean(abs(Y_wpe(:, FrameNo, reference_mic)).^2, 'all');
    ...
    % initialize variables for parfor loop %
    S_WPD_frequencywise = w_bar'*Y_bar;
    error_frequencywise = zeros(MicNum, (ini_frame-(start_ini_frame+L-1)+1))
    ...
    for FrameNo = start_ini_frame+L-1:ini_frame
        ...
        error_frequencywise(:, FrameNo-(start_ini_frame+L-1)+1) = Y_delay_choose' - S_WPD_choose'*weight;
        k_RLS = lambda^-1*P*S_WPD_choose/(1 + lambda^-1*S_WPD_choose'*P*S_WPD_choose);
        weight = weight + k_RLS*error_frequencywise(:, FrameNo-(start_ini_frame+L-1)+1).';
