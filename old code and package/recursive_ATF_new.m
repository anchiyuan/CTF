...
NWIN = points_rir;
NFFT = points_rir;
hopsize = points_rir;    % OverlapLength = NWIN - hopsize

% RIR 轉頻域 %
h_transpose = h(:, 1:NWIN).';
[H, freqs_vector, H_t_vector] = stft(h_transpose, fs, Window=rectwin(NWIN), OverlapLength=NWIN-hopsize, FFTLength=NFFT, FrequencyRange='onesided');
...
xlim([0 fs/2])
H_yaxis_underlimit = min(H_dB(2:end, :, look_mic)) - 5;
H_yaxis_upperlimit = max(H_dB(2:end, :, look_mic)) + 5;
ylim([H_yaxis_underlimit H_yaxis_upperlimit])
title('H')
xlabel('frequency(Hz)')
ylabel('magnitude(dB)')
...
a_ini_tdomain = ifft(a_ini_sym, NWIN, 2, 'symmetric');
...
for FrameNo = ini_frame+1:NumOfFrame
    for n = 1:frequency
        ...
        if abs(s_hat(:, n)) > shat_thres_speech    % 限制 speech source 的錯誤更新
    ...
    elseif strcmp(a_plot_mode, 'time')
        ...
        a_tdomain = ifft(a_sym, NWIN, 2, 'symmetric');
...
a_fin_tdomain = ifft(a_fin_sym, NWIN, 2, 'symmetric');
...
% save .wav 檔 %
shat_tdomain_filename_str = ['shat_tdomain_alpha=', string(alpha), '.wav'];
